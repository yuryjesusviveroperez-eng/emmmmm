...
const CANCIONES = [
  { src: 'only.mp3', nombre: 'Only', autor: 'Lee Hi' },
  { src: 'count-on-me.mp3', nombre: 'Count On Me', autor: 'Bruno Mars' },
  { src: 'its-you.mp3', nombre: "It's You", autor: 'Ali Gatie' },
  { src: 'amtrak.mp3', nombre: 'Amtrak', autor: '—' },
];
...
/* =========================================================
   9. EFECTOS VISUALES (estrellas, pétalos, corazones, fuegos)
========================================================= */
const Efectos = {
  ...
  crearEstrellas() {
    ...
    for (let i = 0; i < CONFIG.totalEstrellas; i++) {
      ...
      s.addEventListener('click', () => Constelaciones.alternarEstrella(s));
      frag.appendChild(s);
    }
...
const Reproductor = {
  ...
  alFallar() {
    this.tituloEl.textContent = CANCIONES[state.cancionActual].nombre;
    this.autorEl.textContent = 'Agrega el archivo ' + CANCIONES[state.cancionActual].src + ' junto a index.html';
  ...
  dibujarVisualizador() {
    if (!this.analizador || !this.reproduciendo) return;
